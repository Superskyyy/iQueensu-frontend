...
class SearchPage extends Component {
    ...
    modifyFilterLogics = (event) => {
        const fieldAndValues = event.target.id;
        const checked = event.target.checked;

        const res = [
            fieldAndValues.substring(0, fieldAndValues.indexOf(" ")),
            fieldAndValues.substring(fieldAndValues.indexOf(" ") + 1),
        ];
        ...
        if (res.length === 2) {
            ...
            if (checked) {
                ...
                if (index > -1) {
                    this.setState(
                        (prevState) => (
                            prevState.filterLogics[field].splice(index, 1),
                            {
                                ...prevState,
                                filterLogics: {
    ...
    handleFilterResult = (logics) => {
        ...
        for (const property in logics) {
            if (filteredResults.length === 0) {
                // Intersection properties, Union element in the same property, using lodash lib
                if (logics[property].length > 0) {
                    for (const logicIndex in logics[property]) {
                        // Actual filtering logic
                        filteredResults = filteredResults.concat(
                            this.filter(logics[property][logicIndex], this.state.searchResults)
                        );
                    }
                }
            } else {
    ...
    refreshDisplayedResults = (logics) => {
        ...
        for (const logicIndex in logics) {
    ...
    objectValues = (value) => {
        return Object.values(value).reduce(
            (string, val) =>
                string +
                (typeof val === "object" && val !== null ? this.strOp(this.objectValues(val)) : this.strOp(val))
        );
    };
    ...
    render() {
        return (
            <div className={classes.coursePageDashBoard}>
                <Fragment>
                    <div className={classes.coursePageLeftPart}></div>
                    ...
                    <div className={classes.coursePageCenterPart}>
                        ...
                        {/* filter panel */}
                        <FilterPanel filterMethod={this.modifyFilterLogics} filterOptions={filterOptions} />

                        <SearchResultPanel results={this.refreshDisplayedResults(this.state.filterLogics)} />
                    </div>

                    <div className={classes.coursePageRightPart}></div>
                </Fragment>
            </div>
        );
    }
